...
class BaseDatasetProcessor(ABC):
    """Base class for dataset processing with guidance or transformers models"""
    ...
    def __init__(self, config: Dict):
        ...
        model_type = self.model_config.get('type', 'guidance')  # Default to guidance for backward compatibility
        if model_type == 'transformers':
            self._initialize_transformers_model()
            self.model = None  # Set to None since we use generate_text directly
        else:
            self.model = self._init_guidance_model()
            
        # Load dataset after model initialization
        self.dataset = self.load_dataset()
    
    def _init_guidance_model(self):
        """Initialize guidance model with proper cache configuration"""
        model_cfg = self.model_config
        
        if model_cfg.get('type') == 'llama.cpp':
    ...
    def generate_text(self, prompt, max_new_tokens=1024, stop_strings=None):
        ...
        if not hasattr(self, 'transformers_model'):
            raise ValueError("Transformers model not initialized. Set model type to 'transformers' in config.")
            
        inputs = self.tokenizer(prompt, return_tensors="pt").to(self.transformers_model.device)
    ...
    def load_examples(self):
        ...
        if self.config.get('examples_path'):

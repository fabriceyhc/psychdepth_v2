...
def evaluate_model(evaluator, model, signature_name, module_name, num_demos, trainset):
    ...
    save_path = f'./story_eval/dspy/multiscore/optimized_prompts/MIPROv2_{module_name}-{signature_name}_demos={num_demos}.json'
...
def main():
    ...
    try:
        ...
        # Save results
        pd.DataFrame(results).to_csv("./story_eval/dspy/multiscore/optimized_prompts/summary.csv")
...
if __name__ == "__main__":
    # CUDA_VISIBLE_DEVICES=1 python -m story_eval.dspy.multiscore.annotate
    main()